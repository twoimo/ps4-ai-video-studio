...
export async function generateGrokImagineFactory(job, script, runId, onProgress = async () => {}, deps = {}) {
  ...
  for (const segment of script.segments) {
    await onProgress(Math.round(((segment.index - 1) / FACTORY_CLIP_COUNT) * 70), `${segment.index}번 스틸`);
  ...
  const composed = deps.compose
    ? await deps.compose({ jobDir, script, clipPaths: clips.map((clip) => join(inputClipsDir, clip.output.replace(/^clips\//, ""))) })
    : await composeGrokImagine({
      jobDir,
      script,
