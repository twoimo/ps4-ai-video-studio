...
> 이 저장소는 현재 **제출 준비 중인 엔지니어링 프로토타입**입니다. Gemini 또는 FLUX 3가 만든 서로 다른 다중 클립으로 시작해 최종 MP4까지 완주한 실서비스 E2E 증거는 아직 없습니다. 기술 증거 검사는 콘텐츠의 의미 품질이나 게시 적합성을 판정하지 않으며, 아래의 상태와 한계를 숨기지 않습니다.
...
| Gemini | 전용 Chrome 프로필/CDP 자동화와 쿼터 모니터 구현. 2026-08-12 저장 스냅샷은 쿼터 차단이며 완전한 생성 E2E 미검증 |
| FLUX 3 | 공식 BFL API용 `local-video` 어댑터, 비용 상한, 체크포인트·재개, 다운로드 검증 구현. 2026-08-12 확인 당시 API 키 미설정·대시보드 잔액 0이며 유료 호출 및 생성 E2E 미수행 |
| 최종 판정 | 로컬 편집 산출물은 있으나 실제 승인 provider의 고유 다중 클립 + 출처 결속 + 요구 reviewer payload가 모두 충족된 제출용 통과 run은 없음 |
