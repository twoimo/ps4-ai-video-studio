...
test("studio HTML is a shorts grid first with factory default create", async () => {
  ...
  assert.match(css, /--thumb-h:\s*calc\(100dvh - var\(--header\) - var\(--title\)\)/);
  assert.match(css, /--col:\s*min\(calc\(var\(--thumb-h\) \* 9 \/ 16\),\s*100%\)/);
  assert.match(css, /repeat\(auto-fill,\s*minmax\(min\(100%,\s*var\(--col\)\),\s*var\(--col\)\)\)/);
  assert.match(css, /\.shorts-grid\s*\{[^}]*justify-content:\s*space-evenly/);
