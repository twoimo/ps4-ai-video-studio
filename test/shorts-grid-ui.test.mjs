...
test("watch feed snaps 9:16 masters and leaves drafts on the grid", async () => {
  ...
  assert.match(css, /\.watch-slide\s*\{[^}]*height:\s*100%/);
  assert.match(css, /\.watch-slide\s*\{[^}]*min-height:\s*100%/);
  ...
  assert.match(css, /@media \(max-width:\s*860px\)[\s\S]*\.watch-slide[\s\S]*width:\s*100%[\s\S]*height:\s*100%/);
  assert.match(css, /@media \(max-width:\s*860px\)[\s\S]*\.watch-stage[\s\S]*width:\s*100%[\s\S]*height:\s*100%/);
  assert.equal(/@media \(max-width:\s*860px\)[\s\S]*100svh/.test(css), false);
