...
test("studio HTML is a shorts grid first with factory default create", async () => {
  ...
  assert.match(app, /if \(dy\) closeOpenWatchInspect\(\)/);
...
test("library and overlays fill the viewport instead of a phone column", async () => {
  ...
  assert.match(css, /\.watch-feed\s*\{[^}]*top:\s*0/);
  assert.match(css, /\.watch-feed\s*\{[^}]*right:\s*0/);
  assert.match(css, /\.watch-feed\s*\{[^}]*bottom:\s*0/);
  assert.match(css, /\.watch-feed\s*\{[^}]*left:\s*0/);
...
test("watch feed snaps 9:16 masters and leaves drafts on the grid", async () => {
  ...
  assert.match(css, /\.watch-scroller\s*\{[^}]*scroll-snap-stop:\s*always/);
  assert.match(css, /\.watch-scroller\s*\{[^}]*scroll-behavior:\s*auto/);
  assert.match(css, /\.watch-scroller\s*\{[^}]*overscroll-behavior-y:\s*contain/);
  assert.match(css, /\.watch-scroller\s*\{[^}]*-webkit-overflow-scrolling:\s*touch/);
  assert.match(css, /\.watch-slide\s*\{[^}]*height:\s*var\(--watch-h,\s*100svh\)/);
  assert.match(css, /\.watch-slide\s*\{[^}]*min-height:\s*var\(--watch-h,\s*100svh\)/);
  assert.match(css, /\.watch-slide\s*\{[^}]*max-height:\s*var\(--watch-h,\s*100svh\)/);
  assert.match(css, /\.watch-slide\s*\{[^}]*flex:\s*0 0 auto/);
  assert.match(css, /\.watch-slide\s*\{[^}]*overflow:\s*hidden/);
  assert.match(css, /\.watch-slide\s*\{[^}]*contain:\s*layout paint/);
  assert.match(css, /\.watch-slide\s*\{[^}]*isolation:\s*isolate/);
  ...
  assert.match(css, /\.watch-stage\s*\{[^}]*min-height:\s*var\(--watch-h,\s*100svh\)/);
  assert.match(css, /\.watch-stage\s*\{[^}]*max-height:\s*var\(--watch-h,\s*100svh\)/);
  assert.match(css, /\.watch-stage\s*\{[^}]*flex:\s*0 0 auto/);
  assert.match(css, /\.watch-stage\s*\{[^}]*overflow:\s*hidden/);
  assert.equal(/\.watch-stage\s*\{[^}]*height:\s*100dvh/.test(css), false);
  assert.match(css, /\.watch-stage video,\s*\.watch-stage \.watch-poster\s*\{[^}]*position:\s*absolute/);
  assert.match(css, /\.watch-stage video,\s*\.watch-stage \.watch-poster\s*\{[^}]*inset:\s*0/);
  assert.match(css, /\.watch-inspect\s*\{[^}]*height:\s*var\(--watch-h,\s*100svh\)/);
  ...
  assert.match(css, /@media \(max-width:\s*860px\)[\s\S]*\.watch-slide[\s\S]*height:\s*var\(--watch-h,\s*100svh\)/);
  assert.match(css, /@media \(max-width:\s*860px\)[\s\S]*\.watch-stage[\s\S]*height:\s*var\(--watch-h,\s*100svh\)/);
  assert.match(css, /@media \(max-width:\s*860px\)[\s\S]*object-fit:\s*cover/);
  assert.match(css, /body\.watch-open\s*\{[^}]*overflow:\s*hidden/);
  assert.match(css, /\.watch-feed\s*\{[^}]*overscroll-behavior:\s*none/);
  assert.match(css, /\.watch-slide\s*\{[^}]*overscroll-behavior:\s*none/);
  assert.match(css, /\.watch-stage\s*\{[^}]*overscroll-behavior:\s*none/);
  ...
  assert.match(css, /@media \(min-width:\s*861px\)[\s\S]*\.watch-menu/);
  ...
  assert.match(css, /\.watch-slide\.inspect-open \.watch-inspect/);
  ...
  assert.match(app, /ArrowDown[\s\S]*scrollBy\(\{\s*top:\s*scroller\.clientHeight,\s*behavior:\s*"auto"\s*\}\)/);
  assert.match(app, /ArrowUp[\s\S]*scrollBy\(\{\s*top:\s*-scroller\.clientHeight,\s*behavior:\s*"auto"\s*\}\)/);
  const bindScroller = app.slice(app.indexOf("function bindWatchScroller"), app.indexOf("function activateWatchSlide"));
  assert.equal(bindScroller.includes("Math.abs(dy) > 50"), false);
  ...
  assert.match(app, /Math\.round\(root\.clientHeight\)/);
  assert.match(app, /scrollTop = Math\.round\(scroller\.scrollTop \/ h\) \* h/);
  assert.match(app, /scrollend/);
  ...
  assert.match(app, /playsinline webkit-playsinline loop preload/);
  ...
  assert.match(app, /class="watch-inspect-dismiss"/);
  ...
  assert.match(bindSlide, /toggleWatchInspect\(slide\)/);
  assert.equal(bindSlide.includes('class="watch-materials"'), false);
...
test("watch inspector saves drafts and freezes regen", async () => {
  ...
  assert.match(css, /\.watch-inspect\s*\{[^}]*height:\s*var\(--watch-h,\s*100svh\)/);
  assert.match(css, /@media \(max-width:\s*860px\)[\s\S]*\.watch-inspect[\s\S]*translateX\(100%\)/);
  assert.match(css, /@media \(max-width:\s*860px\)[\s\S]*\.inspect-open \.watch-inspect[\s\S]*translateX\(0\)/);
  assert.equal(css.includes("40dvh"), false);
  assert.match(pipeline, /export async function saveJobDraft/);
