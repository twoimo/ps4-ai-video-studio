...
test("studio HTML is a shorts grid first with factory default create", async () => {
  ...
  assert.match(css, /grid-template-rows:\s*repeat\(var\(--n\),\s*minmax\(0,\s*1fr\)\)/);
  assert.match(css, /\.shorts-grid\s*\{[^}]*width:\s*100%/);
  assert.match(css, /\.shorts-grid\s*\{[^}]*height:\s*100%/);
  assert.match(css, /\.short-card\s*\{[^}]*container-type:\s*size/);
  assert.match(css, /\.short-card-thumb\s*\{[^}]*width:\s*min\(100%,\s*calc\(100cqh \* 9 \/ 16\)\)/);
  assert.match(css, /\.short-card-thumb\s*\{[^}]*height:\s*min\(100%,\s*calc\(100cqw \* 16 \/ 9\)\)/);
  assert.match(css, /\.short-card-thumb\s*\{[^}]*border-radius:\s*0/);
  assert.match(css, /\.short-card-thumb\s*\{[^}]*border:\s*0/);
  assert.equal(css.includes("--cols:"), false);
  assert.equal(css.includes("--thumb-h:"), false);
  ...
  assert.match(app, /setProperty\("--n"/);
  assert.match(app, /Math\.ceil\(Math\.sqrt\(1 \+ /);
...
test("home chrome drops dashboard dump and keeps a Shorts grid", async () => {
  ...
  assert.match(home, /class="brand-mark"[\s\S]*viewBox="0 0 24 24"[\s\S]*width="22"[\s\S]*height="22"/);
  assert.match(home, /<rect x="5.2" y="2.2" width="13.6" height="19.6" rx="3.6"/);
  ...
  assert.match(css, /\.brand-mark\s*\{[^}]*flex:\s*0 0 auto/);
  assert.equal(/\.brand-mark\s*\{[^}]*width:\s*24px/.test(css), false);
...
test("watch feed snaps 9:16 masters and leaves drafts on the grid", async () => {
  ...
  assert.match(app, /if \(state\.view === "watch"\) \{\s*openHome\(\);/);
