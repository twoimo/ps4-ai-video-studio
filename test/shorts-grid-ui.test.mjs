...
test("watch feed snaps 9:16 masters and leaves drafts on the grid", async () => {
  ...
  assert.match(app, /ArrowDown[\s\S]*scrollBy\(\{\s*top:\s*h,\s*behavior:\s*"auto"\s*\}\)/);
  assert.match(app, /ArrowUp[\s\S]*scrollBy\(\{\s*top:\s*-h,\s*behavior:\s*"auto"\s*\}\)/);
  assert.match(app, /ArrowDown[\s\S]*afterWatchSnap\(\)/);
