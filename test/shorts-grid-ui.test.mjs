...
test("watch feed snaps 9:16 masters and leaves drafts on the grid", async () => {
  ...
  assert.match(css, /\.watch-inspect\s*\{[^}]*height:\s*100svh/);
  ...
  assert.match(css, /@media \(max-width:\s*860px\)[\s\S]*\.watch-feed[\s\S]*width:\s*100%[\s\S]*height:\s*100%/);
  assert.match(css, /@media \(max-width:\s*860px\)[\s\S]*\.watch-slide[\s\S]*height:\s*100dvh[\s\S]*height:\s*100svh/);
  assert.match(css, /@media \(max-width:\s*860px\)[\s\S]*\.watch-stage[\s\S]*height:\s*100svh/);
